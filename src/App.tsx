...
export default function App() {
  ...
  const subject = useMemo(() => {
    return `(${state.company}　${state.name})　${state.month}月度月末報告`;
  }, [state]);

  const attachmentsText = useMemo(() => {
    return state.includeWorkRecord ? '勤務表・交通費' : '交通費';
  }, [state.includeWorkRecord]);

  const body = useMemo(() => {
    return `総務　ご担当者様

お疲れ様です。${state.name}です。
${attachmentsText}の明細を送ります。

どうぞよろしくお願いいたします。

__
${state.name}`;
  }, [state.name, attachmentsText]);

  // --- Handlers ---
  const recipient = 'soumu@senjusoft.co.jp';
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex items-center gap-3 bg-gray-50 p-3.5 rounded-xl opacity-50 cursor-not-allowed">
